...
function Imagesroll({offtop}) {
    ...
    return (
        <>
            <div className='flex mx-auto flex-col h-auto w-[90%] xl:h-[calc(100vh-15rem)] mt-4'>
                {value ? <div className={"group flex items-center justify-center place-items-center w-full h-[calc(100vw*0.5625)] text-center overflow-hidden shadow-lg " + (offtop ? '':'relative')}>
                    <img className="w-full h-full shadow-lg bg-gradient-to-r from-gray-500 to-gray-400 aspect-video" src={image[value]} alt="..." />
                    <div className={"absolute py-auto hidden md:bottom-[40%] inset-x-0 text-center leading-4 justify-between "+(offtop ? '':'md:group-hover:flex')}>
                    ...
                    : 
                    <div className={"flex flex-col group w-full instcount2 items-center justify-center text-center align-middle border-none h-[calc(100vw*0.7)] sm:h-[calc(100vw*0.5625)] navimg " + (offtop?'':'relative')}>
                        ...
                        <div className={"absolute hidden py-auto md:bottom-[40%] inset-x-0 text-center leading-4 group-hover:justify-between "+(offtop?'':'md:group-hover:flex')}>
                            <button className='flex h-12 w-12 translate-y-1/2 text-white z-10 transition bg-opacity-50 text-xl  rounded-full bg-blue-300 items-center justify-center font-extrabold hover:bg-blue-900 m-2' onClick={() => { setValue(value === 0 ? 5 : value - 1) }}>&larr;</button>
